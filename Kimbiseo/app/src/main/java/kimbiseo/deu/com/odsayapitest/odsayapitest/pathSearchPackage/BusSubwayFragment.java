...
public class BusSubwayFragment extends Fragment{
// 모든 길 찾기 프라그먼트
        ...
        public static ArrayList<ArrayList<String>> onoffList=new ArrayList<ArrayList<String>>();
        // 각 어레이 리스트 인덱스는 각 정보 보기 버튼, 내부 어레이 리스트는 차례대로 승차,하차 정류장 순.
        public static ArrayList<String> temponoffList=new ArrayList<String>();
        int whatbutton=-1;
        int count=-1;

        public static BusSubwayFragment busresultfragment;
        public BusSubwayFragment() {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
                ...
                LinearLayout layout = (LinearLayout)inflater.inflate(R.layout.bussubwayresult_fragment,container,false);
                tableLayout=(TableLayout)layout.findViewById(R.id.bussubwayresultTable); // 테이블 레이아웃
                try {
                        MakePathinfoTable();
        ...
        private void MakeTableRow(String totalTime, String totalDistance, String payment, ArrayList<String> temponoffList){ // 테이블 로우에 행을 추가하는 메서드
        ...
        private void MakePathinfoTable() throws JSONException { // 각 테이블 행에 들어갈 데이터 생성 부.
                for (int rowcount = 0; rowcount < DataSingleton.INSTANCE.BigPathBusSubway.size(); rowcount++) {
                    ArrayList<String> temponoffList=new ArrayList<String>();

                    JSONObject PathObject=DataSingleton.INSTANCE.BigPathBusSubway.get(rowcount);
